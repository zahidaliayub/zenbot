  ...
  return [
    ...
    function (tick, trigger, rs, cb) {
      if (get('command') !== 'run' || !c.gdax_key) {
        ...
        if (c.reporter_cols.indexOf('timestamp') === -1) {
          ...
          if (get('command') === 'run') {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('No trader API key provided. Starting in advisor mode. --Zen').yellow, {feed: 'trader'})
          }
        }
        if (get('command') === 'sim') {
      ...
      client.getAccounts(function (err, resp, accounts) {
        ...
        if (balance_sig !== last_balance_sig) {
          if (first_run) {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, '"Starting REAL trading! Hold on to your butts!" --Zen'.cyan, ' Balance:'.grey, n(rs.balance[rs.asset]).format('0.000').white, rs.asset.grey, n(rs.balance[rs.currency]).format('0.00').yellow, rs.currency.grey, {feed: 'exchange'})
          }
    ...
    function (tick, trigger, rs, cb) {
      ...
      get('ticks').load(get('app_name') + ':' + rs.rsi_tick_id, function (err, rsi_tick) {
        ...
        if (!rs.rsi) {
          if (!rs.rsi_warning) {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('no ' + rs.rsi_period + ' RSI for tick ' + rs.rsi_tick_id).red, {feed: 'trader'})
          }
          rs.rsi_warning = true
        }
        else if (rs.rsi.samples < c.rsi_periods) {
          if (!rs.rsi_warning) {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, (rs.rsi_period + ' RSI: not enough samples for tick ' + rs.rsi_tick_id + ': ' + rs.rsi.samples).red, {feed: 'trader'})
          }
          rs.rsi_warning = true
        }
        else {
        ...
        if (trend !== rs.trend) {
          get('logger').info('trader', get_tick_str(tick.id), c.default_selector, 'RSI:'.grey + rs.rsi.ansi, ('trend: ' + rs.trend + ' -> ' + trend).yellow, {feed: 'trader'})
    ...
    function (tick, trigger, rs, cb) {
      ...
      if (get('command') !== 'sim' && tick.time < start) {
        get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('skipping historical tick ' + tick.id).grey, {feed: 'trader'})
        return cb()
      }
      if (rs.trend && !rs.trend_warning) {
        ...
        if (!rs.market_price) {
          get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('no market_price to act on trend: ' + rs.trend + '!').red, {feed: 'trader'})
        }
      ...
      if (rs.trend && rs.balance && rs.market_price) {
        ...
        if (rs.trend === 'DOWN') {
          ...
          if (rs.last_action_time && tick.time - rs.last_action_time <= rs.min_sell_wait) {
            if (!rs.sell_warning) {
              get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('too soon to sell after ' + rs.last_op + '! waiting ' + get_duration(n(rs.min_sell_wait).subtract(n(tick.time).subtract(rs.last_action_time)).multiply(1000).value())).red, {feed: 'trader'})
            }
          ...
          if (!rs.action_warning) {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('attempting to sell ' + n(size).format('0.00000000') + ' ' + rs.asset + ' for ' + format_currency(n(size).multiply(rs.market_price).value(), rs.currency) + ' ' + rs.currency).yellow, {feed: 'trader'})
          }
          ...
          if (rs.last_action_time && tick.time - rs.last_action_time <= rs.min_buy_wait) {
            if (!rs.buy_warning) {
              get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('too soon to buy after ' + rs.last_op + '! waiting ' + get_duration(n(rs.min_buy_wait).subtract(n(tick.time).subtract(rs.last_action_time)).multiply(1000).value())).red, {feed: 'trader'})
            }
            rs.buy_warning = true
            return cb()
          }
          ...
          rs.op = 'buy'
          if (!rs.action_warning) {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('attempting to buy ' + n(size).format('0.00000000') + ' ' + rs.asset + ' for ' + format_currency(n(size).multiply(rs.market_price).value(), rs.currency) + ' ' + rs.currency).yellow, {feed: 'trader'})
          }
          rs.action_warning = true
        }
        else {
          // unknown trend
          get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('unkown trend (' + rs.trend + ') aborting trade!').red, {feed: 'trader'})
          return cb()
        }
        // min size
        if (!size || size < rs.min_trade) {
          if (!rs.balance_warning) {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, 'trend: '.grey, rs.trend, ('not enough funds (' + (rs.op === 'sell' ? n(size).format('0.00000000') : format_currency(rs.balance[rs.currency], rs.currency)) + ' ' + (rs.op === 'sell' ? rs.asset : rs.currency) + ') to execute min. ' + rs.op + ' ' + rs.min_trade + ', aborting trade!').red, {feed: 'trader'})
          }
        ...
        if (rs.min_performance && rs.performance !== null && rs.performance < rs.min_performance) {
          if (!rs.perf_warning) {
            get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('aborting ' + rs.op + ' due to low perf. = ' + n(rs.performance).format('0.000')).red, {feed: 'trader'})
          }
        ...
        if (client) {
          ...
          get('logger').info('trader', get_tick_str(tick.id), c.default_selector, ('Relax! This is a simulated trade! No real transaction will take place. --Zen').yellow, {feed: 'trader'})
          rs.sim_warning = true
        }
        if (rs.op === 'buy') {
